...
OPTS="$OPTS -drive id=disk0,if=virtio,cache=none,format=qcow2,file=suse.qcow2 "

#  installer(actual disc)
OPTS="$OPTS -drive id=cd0,if=none,format=raw,readonly,file=suse.iso"
